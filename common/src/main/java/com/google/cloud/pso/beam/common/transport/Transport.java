...
/** Represents a transport object being propagated by the different components of a pipeline. */
public sealed interface Transport
    permits EventTransport, ErrorTransport, AggregationResultTransport, AggregationTransport {
  static final String EVENT_TIME_KEY = "eventTimestamp";
  static final String SCHEMA_ATTRIBUTE_KEY = "schemaAttribute";
   ...
   */
  default Optional<Long> getTransportEpochInMillis() {
    return Optional.ofNullable(this.getHeaders().get(EVENT_TIME_KEY))
        .map(Transport::fromStringTimestamp)
