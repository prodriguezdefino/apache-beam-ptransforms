...
public class CreateSuccessFileTest {
  ...
  private void createTestPipeline(
    ...
    TupleTag<GenericRecord> dataToBeIngestedTag =
        WriteFormatToFileDestination.CaptureDataOnWindowSignals
            ...
            .apply(
                ParDo.of(
                        new WriteFormatToFileDestination.CaptureDataOnWindowSignals<>(
                            dataOnWindowSignalsTag, dataToBeIngestedTag))
                    .withOutputTags(dataToBeIngestedTag, TupleTagList.of(dataOnWindowSignalsTag)));

    PCollection<String> fileNames =
        capturedData
            .get(dataToBeIngestedTag)
            .setCoder(coder)
            .apply(
                FileIO.<GenericRecord>write()
                    .via(
                        ParquetIO.sink(ComposeFilesTest.SCHEMA)
  ...
  public void testSuccessFileOnEmptyWindowHourly() throws IOException {
    ...
    Assert.assertTrue(
        resourceList
            .get(0)
            .endsWith(Instant.now().toDateTime().hourOfDay().getAsText() + "/_SUCCESS"));
  }
}
