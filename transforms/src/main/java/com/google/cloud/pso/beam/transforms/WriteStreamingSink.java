...
public class WriteStreamingSink
        extends PTransform<PCollection<EventTransport>, PDone> {
  ...
  public void validate(PipelineOptions options) {
    ...
    Preconditions.checkArgument(
            this.outputTopic != null, "An output topic should be provided");
  }
                ...
                .apply("WriteToKafka",
                        KafkaIO.<byte[], byte[]>writeRecords()
                                ...
                                .withProducerConfigUpdates(
                                        ImmutableMap.of("max.request.size", "6291456")));
        break;
      }
      case PUBSUB: {
        input
                .apply("CreatePubsubMessage",
                        MapElements
                                .into(TypeDescriptor.of(PubsubMessage.class))
                                .via(kvs
                                        -> new PubsubMessage(
                                        kvs.getData(), kvs.getHeaders(), kvs.getId())))
      ...
      case PUBSUBLITE: {
        ...
        input
                .apply("CreatePubSubLiteMessages",
                        ParDo.of(new CreatePubSubLiteMessage()))
                .apply("WriteToPubSubLite",
                        PubsubLiteIO.write(
                                PublisherOptions
                                        .newBuilder()
  ...
  static class CreatePubSubLiteMessage
          extends DoFn<EventTransport, PubSubMessage> {
    ...
    public void processElement(ProcessContext context) {
      ...
      var msgBuilder
              = PubSubMessage
                      ...
                      .putAllAttributes(
                              dataPayload.getHeaders().entrySet()
                                      .stream()
                                      .map(entry -> KV.of(
                                      entry.getKey(),
                                      AttributeValues
      ...
      // if the element has a ts we used that, if not then we just return the element without that info
      context.output(context.element()
              .getEventEpochInMillis()
              .map(ts -> msgBuilder.setEventTime(
  ...
  static class CreateKafkaRecord
          extends DoFn<EventTransport, ProducerRecord<byte[], byte[]>> {
